...
class my_gin(nn.Module):
    def __init__(self, in_dim=512, n_dim=512,n_head=8,d_k=64,d_v=64,dropout=0.1):
        ...
        self.At = nn.Linear(d_v*2, d_v, bias=False)
    ...
    def aggerate(self, data, index):
        ...
        sz_b, len_data= data.size(0), data.size(1)
        q = self.w_qs(data).view(sz_b, len_data, n_head, d_k)
        k = self.w_ks(data).view(sz_b, len_data, n_head, d_k)
        v = self.w_vs(data).view(sz_b, len_data, n_head, d_v)
        ...
        attn = attn.masked_fill(index == 0, -10)

        attn = self.dropout(F.softmax(attn, dim=-1))
        q = torch.matmul(attn, v)
        q = q.transpose(1, 2).contiguous().view(sz_b, len_data, -1)
...
class Encoder_p(nn.Module):
    ...
    def forward(self, src_seq,index ,return_attns=False):
        ...
        diag = ((torch.matmul(index, index) * torch.eye(output.size(1)).cuda().unsqueeze(0)) > 0).float()
        index = index + diag
